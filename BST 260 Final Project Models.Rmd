...
Among the three options, we picked our final model based on the best test performance. We were concerned by the large imbalance in classes in our data set. We only had 7.4% of "Bad" laon status. Therefore, we decided to evalute our model based on the AUC performance because this metric is less affected by the imbalance in the classes. We kept about a fifth of a data (200,000 records) for testing and used the rest for training our models. We trained our candidate models using 10-fold cross-validation (on the training set) to obtain the optimal tuning parameters and finally tested their performance on the test data set. 
...
models = c("ridge", "lasso", "enet", "xgbTree", "C5.0Cost", "svmLinearWeights")
models = c("xgbTree", "C5.0Cost", "svmLinearWeights")
...
# save(fit.ridge, fit.lasso, fit.enet,
#      fit.xgb, fit.ccost, fit.svm, file = "./models.RData")
#save(fit.xgb, fit.ccost, fit.svm, file = "./models.RData")
```
...
The AUC for all three models are similar. However, lasso model (with \(\alpha=1\) and \(\lambda=6.103516e-05\)) had the largest AUC. Therefore, we decided to use the elastic net model (with \(\alpha=1\) and \(\lambda=6.103516e-05\)) as our final model and retrained the elastic net model with these parameters using all data.
```{r}
...
Here is our final elastic net model (\(\alpha=1\) and \(\lambda=6.103516e-05\)) with the following regression coefficients:
```{r}
#setwd('/Users/shuheimiyasaka/Google Drive/Harvard/Courses/BST 260/BST-260-Final-Project')
