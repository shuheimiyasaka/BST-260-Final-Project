...
* In fit3, the OR of grade is 1.53 (but this result is rather obvious)
* Maybe we can also run regression to see factors associated with high grades? (e.g. seems like employment length increasing grade which is a little counterintuitive)
...
dat <- dat %>%
  mutate(annual_inc_cat = case_when(annual_inc < 25000 ~ 1, 
                                    annual_inc >= 25000 & annual_inc <=49999 ~ 2,
                                    annual_inc >=50000 & annual_inc <= 99999 ~3,
                                    annual_inc >= 100000 & annual_inc <= 199999 ~4,
                                    annual_inc >= 200000 ~5))

fit4 <- lm(grade_2 ~ annual_inc_cat + emp_length_2, data=dat)
summary(fit4)
```
