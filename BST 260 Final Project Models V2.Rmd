...
```{r}
loan.dat.stdized <- loan.dat

meta_loans <- funModeling::df_status(loan.dat.stdized, print_results = FALSE)
numeric_cols <- meta_loans$variable[meta_loans$type == 'numeric']

for (col_name in numeric_cols){
  loan.dat.stdized[, col_name] =
    (loan.dat.stdized[, col_name] - mean(loan.dat.stdized[, col_name]))/sd(loan.dat.stdized[, col_name])
}
