...
#setwd('/Users/shuheimiyasaka/Google Drive/Harvard/Courses/BST 260/BST-260-Final-Project')
#save(loan.dat, loan.dat.original, file = "./loan.dat.v2.RData")
```
...
The AUC for all three models are similar. However, elastic net model (with \(\alpha=0.85\) and \(\lambda=0.0002441406\)) had the largest AUC. Therefore, we decided to use the elastic net model (with \(\alpha=0.85\) and \(\lambda=0.0002441406\)) as our final model and retrained the elastic net model with these parameters using all data.
```{r}
x = model.matrix(loan_status_bin ~ ., loan.dat)[,-1]
y = loan.dat$loan_status_bin

final_mod = glmnet(x=x, y=y, family = 'binomial', alpha = 0.85, 
                   lambda = 0.0002441406)
setwd('/Users/shuheimiyasaka/Google Drive/Harvard/Courses/BST 260/BST-260-Final-Project')
save(final_mod, file = "./final_model.v2.RData")
```

## Results

Here is our final elastic net model (\(\alpha=0.85\) and \(\lambda=0.0002441406\)) with the following regression coefficients:
```{r}
#setwd('/Users/shuheimiyasaka/Google Drive/Harvard/Courses/BST 260/BST-260-Final-Project')
#load("final_model.v.2RData")
coef(final_mod)
```
